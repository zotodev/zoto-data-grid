...
import { registerDataGridTableController } from "@/components/data-grid-table/internal-controller"
...
interface UseDataGridTableCoreOptions<TData> extends Omit<TableOptions<TData>, "pageCount" | "getCoreRowModel"> {
...
function useDataGridTableController<TData>({
  data,
  columns,
  ...
  manualPagination,
  ...props
}: UseDataGridTableCoreOptions<TData>) {
  ...
  useIsomorphicLayoutEffect(() => {
    if (props.state?.sorting) {
      ...
      if (JSON.stringify(currentSorting) !== JSON.stringify(props.state.sorting)) {
  ...
  useIsomorphicLayoutEffect(() => {
    if (props.state?.columnFilters) {
      const currentFilters = store.getState().columnFilters
      if (JSON.stringify(currentFilters) !== JSON.stringify(props.state.columnFilters)) {
  ...
  const serializeCellsToTsv = React.useCallback(() => {
    ...
    const rowCellMaps = new Map<number, Map<string, ReturnType<Row<TData>["getVisibleCells"]>[number]>>()
  ...
  const onCellsCopy = React.useCallback(async () => {
    ...
    try {
      ...
      toast.success(`${selectedCellsArray.length} cell${selectedCellsArray.length !== 1 ? "s" : ""} copied`)
    } catch (error) {
  ...
  const onCellsCut = React.useCallback(async () => {
    ...
    try {
      ...
      toast.success(`${selectedCellsArray.length} cell${selectedCellsArray.length !== 1 ? "s" : ""} cut`)
    } catch (error) {
    ...
    async (expandRows = false) => {
      ...
      try {
        ...
        if (updates.length > 0) {
          ...
          if (cellsSkipped > 0) {
            toast.success(`${cellsUpdated} cell${cellsUpdated !== 1 ? "s" : ""} pasted, ${cellsSkipped} skipped`)
          } else {
          ...
          toast.error(`${cellsSkipped} cell${cellsSkipped !== 1 ? "s" : ""} skipped pasting for invalid data`)
        }
  ...
  const onRowsDelete = React.useCallback(
    async (rowIndices: number[]) => {
      if (propsRef.current.readOnly || !propsRef.current.onRowsDelete || rowIndices.length === 0) return
    ...
    (direction: NavigationDirection) => {
      ...
      if (newRowIndex !== rowIndex || newColumnId !== columnId) {
        ...
        if (!targetRow) {
          if (rowVirtualizer) {
            const align =
              direction === "up" || direction === "pageup" || direction === "ctrl+up" || direction === "ctrl+home"
                ? "start"
                : direction === "down" ||
                    direction === "pagedown" ||
                    direction === "ctrl+down" ||
  ...
  const onColumnFiltersChange = React.useCallback(
    (updater: Updater<ColumnFiltersState>) => {
      const currentState = store.getState()
      const newColumnFilters = typeof updater === "function" ? updater(currentState.columnFilters) : updater
  ...
  const adjustLayout = React.useMemo(() => {
    ...
    return isFirefox && ((columnPinning.left?.length ?? 0) > 0 || (columnPinning.right?.length ?? 0) > 0)
...
export interface UseDataGridTableOptions<TData> extends Omit<UseDataGridTableCoreOptions<TData>, "filterFns"> {
  filterFns?: Partial<TableOptions<TData>["filterFns"]>
  search?: Record<string, unknown>
  searchDebounceMs?: number
}

type DataGridTableCoreController<TData> = ReturnType<typeof useDataGridTableController<TData>>

export type DataGridTableController<TData> = DataGridTableCoreController<TData>

function getColumnId<TData>(column: ColumnDef<TData>) {
  return column.id ?? ("accessorKey" in column ? String(column.accessorKey) : "")
}

function parseSearchFilterValue<TData>(column: ColumnDef<TData>, value: unknown) {
...
export function useDataGridTable<TData>({
  ...
  const stableData = options.data.length === 0 ? (EMPTY_DATA as TData[]) : options.data
  const initialSortingKey = JSON.stringify(initialState?.sorting ?? [])

  const searchSorting = React.useMemo<SortingState>(
    () =>
      search?.sortBy
        ? [{ id: String(search.sortBy), desc: search.sortOrder === "desc" }]
        : (initialState?.sorting ?? []),
    [initialSortingKey, search?.sortBy, search?.sortOrder]
  )
  ...
  const filterSearchKey = filterableColumns
    .map((column) => {
      ...
      return `${id}:${JSON.stringify(search?.[id])}`
    })
    .join("|")

  const searchColumnFilters = React.useMemo<ColumnFiltersState>(() => {
    if (!search) return []
    ...
    return filterableColumns.flatMap((column) => {
      ...
      return [{ id, value: parseSearchFilterValue(column, value) }]
    })
  }, [filterSearchKey, filterableColumns])

  const onSearchSortingChange = React.useCallback(
    (updater: Updater<SortingState>) => {
      const nextSorting = typeof updater === "function" ? updater(searchSorting) : updater
    ...
    [navigate, searchSorting]
  )

  const onSearchColumnFiltersChange = React.useCallback(
    (updater: Updater<ColumnFiltersState>) => {
      const nextFilters = typeof updater === "function" ? updater(searchColumnFilters) : updater
      ...
      for (const column of filterableColumns) {
        ...
        const filter = nextFilters.find((item) => item.id === id)
      ...
      const changedColumn = filterableColumns.find((column) => {
        const id = getColumnId(column)
        const previousValue = searchColumnFilters.find((item) => item.id === id)?.value
        const nextValue = nextFilters.find((item) => item.id === id)?.value
        return JSON.stringify(previousValue) !== JSON.stringify(nextValue)
      })
      const changedVariant = changedColumn?.meta?.filter?.variant
      const shouldDebounce = changedVariant === "text" || changedVariant === "number"
      const updateSearch = () => {
        navigate({
          to: ".",
          search: (previous: Record<string, unknown>) => ({
      ...
      if (shouldDebounce) {
        debounceRef.current = setTimeout(updateSearch, searchDebounceMs)
      } else {
        updateSearch()
      }
    },
    [filterableColumns, navigate, searchColumnFilters, searchDebounceMs]
  )
  ...
  const hasSearchState = Boolean(search)
  const hasControlledSorting = options.state?.sorting !== undefined
  const hasControlledFilters = options.state?.columnFilters !== undefined
  const tableState = React.useMemo(
    () => ({
      ...options.state,
      ...(hasSearchState && !hasControlledSorting ? { sorting: searchSorting } : {}),
      ...(hasSearchState && !hasControlledFilters ? { columnFilters: searchColumnFilters } : {})
    }),
    [options.state, hasSearchState, hasControlledSorting, hasControlledFilters, searchSorting, searchColumnFilters]
  )

  const controller = useDataGridTableController({
    ...options,
    data: stableData,
    columns,
    initialState,
    filterFns: {
      ...options.filterFns,
      arrIncludesSome: options.filterFns?.arrIncludesSome ?? arrIncludesSome,
      dateFilter: options.filterFns?.dateFilter ?? dateFilter
    },
    state: tableState,
    onSortingChange:
      options.onSortingChange ?? (hasSearchState && !hasControlledSorting ? onSearchSortingChange : undefined),
    onColumnFiltersChange:
      options.onColumnFiltersChange ??
      (hasSearchState && !hasControlledFilters ? onSearchColumnFiltersChange : undefined),
    manualSorting: options.manualSorting ?? hasSearchState,
    manualFiltering: options.manualFiltering ?? hasSearchState,
    manualPagination: options.manualPagination ?? hasSearchState,
    enableMultiSort: options.enableMultiSort ?? !hasSearchState
  })

  registerDataGridTableController(controller.table, controller)

  return React.useMemo(() => ({ table: controller.table }), [controller.table])
}
