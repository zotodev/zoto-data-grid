...
export function getIsFileCellData(item: unknown): item is FileCellData {
  return !!item && typeof item === "object" && "id" in item && "name" in item && "size" in item && "type" in item
}

export function matchSelectOption(value: string, options: { value: string; label: string }[]): string | undefined {
  return options.find(
    (o) =>
      o.value === value ||
...
export function getColumnPinningStyle<TData>(params: {
  ...
  const { column, dir = "ltr", withBorder = false, background = "var(--background)" } = params
...
export function getScrollDirection(direction: string): "left" | "right" | "home" | "end" | undefined {
  if (direction === "left" || direction === "right" || direction === "home" || direction === "end") {
