    ...
    <Sortable value={sorting} onValueChange={onSortingChange} getItemValue={(item) => item.id}>
      <Popover open={open} onOpenChange={setOpen}>
        ...
        >
          ...
          {sorting.length > 0 && (
            <SortableContent render={<ul className="flex max-h-[300px] flex-col gap-2 overflow-y-auto p-1" />}>
              {sorting.map((sort) => (
                <DataTableSortItem
                  key={sort.id}
                  sort={sort}
...
function DataTableSortItem({
  ...
  return (
    <SortableItem
      value={sort.id}
      render={<li id={sortItemId} tabIndex={-1} className="flex items-center gap-2" onKeyDown={onItemKeyDown} />}
    >
      ...
      <SortableItemHandle render={<Button variant="outline" size="icon" className="size-8 shrink-0 rounded" />}>
        <GripVertical />
      </SortableItemHandle>
    </SortableItem>
  )
