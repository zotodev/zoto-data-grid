  ...
  return (
    ...
    >
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger render={<Button dir={dir} variant="outline" size="sm" className="font-normal" onKeyDown={onTriggerKeyDown} disabled={disabled} />}><ListFilter className="text-muted-foreground" />Filter
                          {columnFilters.length > 0 && (
                            <Badge
                              variant="secondary"
...
function DataGridFilterItem<TData>({
  ...
  return (
    <SortableItem value={filter.id} render={<div role="listitem" id={filterItemId} tabIndex={-1} className="flex items-center gap-2" onKeyDown={onItemKeyDown} />}><div className="min-w-[72px] text-center">
                ...
                <PopoverTrigger render={<Button id={fieldTriggerId} aria-controls={fieldListboxId} dir={dir} variant="outline" size="sm" className="w-32 justify-between rounded font-normal" />}><span className="truncate">{columnLabels.get(filter.id)}</span><ChevronsUpDown className="opacity-50" /></PopoverTrigger>
                <PopoverContent
                  id={fieldListboxId}
                  dir={dir}
                  align="start"
...
function DataGridFilterInput<TData>({
    ...
    if (isBetween) {
      ...
      return (
        <Popover open={showValueSelector} onOpenChange={setShowValueSelector}>
          <PopoverTrigger render={<Button id={inputId} aria-controls={inputListboxId} dir={dir} variant="outline" size="sm" className={cn(
  ...
  if (isSelectVariant && selectOptions.length > 0) {
    ...
    if (isMultiValueOperator) {
      ...
      return (
        <Popover open={showValueSelector} onOpenChange={setShowValueSelector}>
          <PopoverTrigger render={<Button id={inputId} aria-controls={inputListboxId} dir={dir} variant="outline" size="sm" className="h-8 w-full justify-start rounded font-normal" />}>{selectedOptions.length === 0 ? (
    ...
    return (
      <Popover open={showValueSelector} onOpenChange={setShowValueSelector}>
        <PopoverTrigger render={<Button id={inputId} aria-controls={inputListboxId} dir={dir} variant="outline" size="sm" className="h-8 w-full justify-start rounded font-normal" />}>{selectedOption ? (
                          <>
