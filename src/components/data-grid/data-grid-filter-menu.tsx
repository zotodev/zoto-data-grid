...
export function DataGridFilterMenu<TData>({ table, disabled, className, ...props }: DataGridFilterMenuProps<TData>) {
  ...
  const onFilterUpdate = React.useCallback(
    (filterId: string, updates: Partial<ColumnFilter>) => {
      table.setColumnFilters((prevFilters) => {
        ...
        return prevFilters.map((filter) => (filter.id === filterId ? { ...filter, ...updates } : filter))
      })
    },
    [table]
  )
  ...
  return (
    <Sortable value={columnFilters} onValueChange={table.setColumnFilters} getItemValue={(item) => item.id}>
      <Popover open={open} onOpenChange={setOpen}>
        ...
        >
          ...
          {columnFilters.length > 0 && (
            <SortableContent
              render={<div role="list" className="flex max-h-[400px] flex-col gap-2 overflow-y-auto p-1" />}
            >
  ...
  return (
    ...
    >
              ...
              <CommandGroup>
                {columns.map((column) => (
                  ...
                  >
                    ...
                    <Check className={cn("ms-auto", column.id === filter.id ? "opacity-100" : "opacity-0")} />
                  </CommandItem>
                ))}
              </CommandGroup>
            </CommandList>
      ...
      <Select
        ...
        value={operator}
        onValueChange={(value) => {
          if (value) onOperatorChange(value as FilterOperator)
        }}
      >
      ...
      <SortableItemHandle render={<Button variant="outline" size="icon" className="size-8 rounded" />}>
        <GripVertical />
      </SortableItemHandle>
    </SortableItem>
  )
...
function DataGridFilterInput<TData>({
  ...
  const debouncedOnChange = useDebouncedCallback((newValue: string | number | string[] | undefined) => {
    onValueChange(newValue)
  }, FILTER_DEBOUNCE_MS)

  const debouncedOnEndValueChange = useDebouncedCallback((newValue: string | number | string[] | undefined) => {
  ...
  const selectOptions = React.useMemo(() => {
    return cellVariant?.variant === "select" || cellVariant?.variant === "multi-select" ? cellVariant.options : []
  }, [cellVariant])
  ...
  if (variant === "date") {
    ...
    if (isBetween) {
      const startDate = localValue && typeof localValue === "string" ? new Date(localValue) : undefined
      const endDate = localEndValue && typeof localEndValue === "string" ? new Date(localEndValue) : undefined
          ...
          <PopoverTrigger
            render={
              <Button
                ...
                variant="outline"
                size="sm"
                className={cn("h-8 w-full justify-start rounded font-normal", !startDate && "text-muted-foreground")}
              />
            }
          >
            <CalendarIcon />
    ...
    const dateValue = localValue && typeof localValue === "string" ? new Date(localValue) : undefined
        ...
        <PopoverTrigger
          render={
            <Button
              ...
              variant="outline"
              size="sm"
              className={cn("h-8 w-full justify-start rounded font-normal", !dateValue && "text-muted-foreground")}
            />
          }
        >
          <CalendarIcon />
          <span className="truncate">{dateValue ? formatDate(dateValue, { month: "short" }) : "Pick a date"}</span>
        </PopoverTrigger>
        <PopoverContent id={inputListboxId} dir={dir} align="start" className="w-auto p-0">
          <Calendar
            autoFocus
                  ...
                  {selectOptions.map((option) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {option.count && <span className="ms-auto font-mono text-xs">{option.count}</span>}
              ...
              <CommandGroup>
                {selectOptions.map((option) => (
                  ...
                  >
                    ...
                    <span className="truncate">{option.label}</span>
                    {option.count && <span className="ms-auto font-mono text-xs">{option.count}</span>}
                    <Check className={cn("ms-auto", value === option.value ? "opacity-100" : "opacity-0")} />
                  </CommandItem>
                ))}
              </CommandGroup>
            </CommandList>
