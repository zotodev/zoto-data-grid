    ...
    >
      <button
        ...
        className={cn(primaryNameCellClassName, {
          ...
          "cursor-default text-muted-foreground no-underline": !value || !onPrimaryNameClick
        })}
      >
        {value || "—"}
      </button>
...
export function LongTextCell<TData>({
  ...
  const onInitialFocus = React.useCallback((): false => {
  ...
  const onWrapperKeyDown = React.useCallback(
    (event: React.KeyboardEvent<HTMLDivElement>) => {
      if (isFocused && !isEditing && !readOnly && event.key.length === 1 && !event.ctrlKey && !event.metaKey) {
...
export function CheckboxCell<TData>({
  ...
  const onCheckboxClick = React.useCallback<NonNullable<React.ComponentProps<typeof Checkbox>["onClick"]>>((event) => {
    event.stopPropagation()
  }, [])

  const onCheckboxMouseDown = React.useCallback<NonNullable<React.ComponentProps<typeof Checkbox>["onMouseDown"]>>(
    (event) => {
      event.stopPropagation()
    },
    []
  ...
  const onCheckboxDoubleClick = React.useCallback<NonNullable<React.ComponentProps<typeof Checkbox>["onDoubleClick"]>>(
    (event) => {
      event.stopPropagation()
    },
    []
...
export function SelectCell<TData>({
  ...
  const containerRef = React.useRef<HTMLDivElement>(null)
  const cellOpts = cell.column.columnDef.meta?.cell
  const options = React.useMemo(() => (cellOpts?.variant === "select" ? cellOpts.options : []), [cellOpts])
  ...
  return (
    ...
    >
      {isEditing ? (
        ...
        <Badge data-slot="grid-cell-content" variant="secondary" className="whitespace-pre-wrap px-1.5 py-px">
          {displayLabel}
        </Badge>
      ) : null}
    </DataGridCellWrapper>
...
export function MultiSelectCell<TData>({
  ...
  const inputRef = React.useRef<HTMLInputElement>(null)
  const cellOpts = cell.column.columnDef.meta?.cell
  const options = React.useMemo(() => (cellOpts?.variant === "multi-select" ? cellOpts.options : []), [cellOpts])
  const optionByValue = React.useMemo(() => new Map(options.map((option) => [option.value, option])), [options])
  const sideOffset = -(containerRef.current?.clientHeight ?? 0)
  ...
  const displayLabels = selectedValues.map((val) => optionByValue.get(val)?.label ?? val).filter(Boolean)
  ...
  return (
    ...
    >
      {isEditing ? (
        <Popover open={isEditing} onOpenChange={onOpenChange}>
          <PopoverTrigger nativeButton={false} render={<div className="absolute inset-0" />} />
          <PopoverContent
            data-grid-cell-editor=""
            align="start"
            sideOffset={sideOffset}
...
export function DateCell<TData>({
  ...
  return (
    ...
    >
      <Popover open={isEditing} onOpenChange={onOpenChange}>
        <PopoverTrigger nativeButton={false} render={<span data-slot="grid-cell-content" />}>
...
export function FileCell<TData>({
  ...
  const onFileCellKeyDown = React.useCallback((event: React.KeyboardEvent<HTMLDivElement>) => {
    if (event.key === "Escape") {
