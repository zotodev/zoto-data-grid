...
export function CheckboxCell<TData>({
  ...
  const onCheckboxMouseDown = React.useCallback<
    NonNullable<React.ComponentProps<typeof Checkbox>["onMouseDown"]>
  >((event) => {
    event.stopPropagation();
  }, []);

  const onCheckboxDoubleClick = React.useCallback<
    NonNullable<React.ComponentProps<typeof Checkbox>["onDoubleClick"]>
  >((event) => {
    event.stopPropagation();
  }, []);
...
export function FileCell<TData>({
  ...
  const onFileCellKeyDown = React.useCallback(
    (event: React.KeyboardEvent<HTMLDivElement>) => {
      if (event.key === "Escape") {
