...
function DataGridSelectHitbox({ htmlFor, children, size, debug }: DataGridSelectHitboxProps) {
  return (
    <div
      className={cn(
        "group -my-1.5 relative h-[calc(100%+0.75rem)] py-1.5",
    ...
    >
      ...
      <label
        htmlFor={htmlFor}
        className={cn("absolute inset-0 cursor-pointer", debug && "border border-red-500 border-dashed bg-red-500/20")}
      />
    </div>
  )
}
...
function DataGridSelectHeader<TData>({ table, hitboxSize, readOnly, debug }: DataGridSelectHeaderProps<TData>) {
...
interface DataGridSelectCellProps<TData> extends Pick<CellContext<TData, unknown>, "row" | "table"> {
  hitboxSize?: HitboxSize
...
function DataGridSelectCell<TData>({
  ...
  const onClick = React.useCallback<NonNullable<React.ComponentProps<typeof Checkbox>["onClick"]>>(
    (event) => {
      if (event.shiftKey) {
...
interface GetDataGridSelectColumnOptions<TData> extends Omit<Partial<ColumnDef<TData>>, "id" | "header" | "cell"> {
  enableRowMarkers?: boolean
  readOnly?: boolean
  hitboxSize?: HitboxSize
  debug?: boolean
